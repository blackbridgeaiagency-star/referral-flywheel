...
export default async function CreatorDashboardPage({ params }: CreatorDashboardPageProps) {
  ...
  if (!creator) {
    console.log(`🚀 Auto-creating creator for company: ${companyId}`);

    creator = await prisma.creator.create({
      data: {
        companyId: companyId,
        productId: companyId, // Use companyId as productId for now
  ...
  // Fetch dashboard data using productId from the creator record
  // ========================================
        ...
        <div className="max-w-2xl w-full bg-[#1A1A1A] border border-red-500/30 rounded-xl p-8">
          <div className="flex items-start justify-between mb-4">
            ...
            <div className="text-right">
              ...
              <p className="text-purple-400 font-mono text-sm">{companyId}</p>
            </div>
          </div>
